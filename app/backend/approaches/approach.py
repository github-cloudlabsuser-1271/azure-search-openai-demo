...
class Approach(ABC):
    """
    Approach class provides an abstract base class for implementing various search approaches using Azure Search and OpenAI services.
    Attributes:
    ...
    Methods:
        ...
        search(top, query_text, filter, vectors, use_text_search, use_vector_search, use_semantic_ranker, use_semantic_captions, minimum_search_score, minimum_reranker_score) -> List[Document]:
            Performs a search using the configured search client and returns a list of qualified documents.
        ...
        get_citation(sourcepage, use_image_citation) -> str:
            Generates a citation string for a given source page, optionally using image citation.
        compute_text_embedding(q) -> VectorizedQuery:
            Computes the text embedding for a given query using the OpenAI client.
        compute_image_embedding(q) -> VectorizedQuery:
            Computes the image embedding for a given query using the vision endpoint.
        run(messages, session_state, context) -> dict[str, Any]:
            Abstract method to be implemented by subclasses for running the approach with given messages and context.
        run_stream(messages, session_state, context) -> AsyncGenerator[dict[str, Any], None]:
            Abstract method to be implemented by subclasses for running the approach with given messages and context, yielding results as a stream.
    """
